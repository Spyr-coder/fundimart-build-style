...
const Categories = () => {
  ...
  useEffect(() => {
    ...
    // Safely retrieve local items
    let storedProducts: Product[] = [];
    let allUsers: User[] = [];
    
    try {
      storedProducts = JSON.parse(localStorage.getItem("fundimart_products") || "[]");
      allUsers = JSON.parse(localStorage.getItem("fundimart_users") || "[]");
    } catch (error) {
      console.error("Error reading categories data from localStorage:", error);
    }
    ...
    // Merge static and dynamic product counts
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 gap-3 md:gap-4">
          {(categoryItems || []).map((category, index) => {
            ...
            return (
              ...
              >
                <div className={`w-10 h-10 md:w-16 md:h-16 mx-auto rounded-xl md:rounded-2xl ${category.color || 'bg-primary/10'} flex items-center justify-center mb-2 md:mb-4 group-hover:scale-110 transition-transform`}>
                  {category.icon && <category.icon className="w-5 h-5 md:w-8 md:h-8" />}
                </div>
